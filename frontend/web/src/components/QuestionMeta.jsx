...
const DIFFICULTY = {
  [Difficulty.EASY]: "border-emerald-500/30 bg-emerald-500/15 text-emerald-400",
  [Difficulty.MEDIUM]: "border-amber-400/30 bg-amber-400/15 text-amber-400",
  [Difficulty.HARD]: "border-rose-500/30 bg-rose-500/15 text-rose-400",
};

export function DifficultyBadge({ difficulty }) {
  const key = (difficulty || Difficulty.MEDIUM).toUpperCase();
  return (
    <span className={`rounded-md border px-2 py-0.5 text-xs font-semibold capitalize ${DIFFICULTY[key] || DIFFICULTY[Difficulty.MEDIUM]}`}>
